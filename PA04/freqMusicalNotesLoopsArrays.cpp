//
//  freqMusicalNotesLoopsArrays.cpp
//  ImplementProj_4
//
//  Created by Charley Teltschik on 3/8/24.
//
...
int main(){
    ...
    for (int nu = 0; nu < k_nOctaves; nu++){
        for (int k = 0; k < k_nHalfTones; k++){
            double f = r_freq * (pow(2, nu)) * pow(dTwelfthRootOfTwo, k);
            double dWavelengthCentimeters = (c*100)/f;
