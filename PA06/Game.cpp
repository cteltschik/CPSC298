...
//
// Complete the initializer list by initializing m_iRowBoat to 0, m_iColumnBoat to0, and
...
/*?? assign '~' to element of 2-D array a_cGrid indexed by iRow
and iColumn ?? */
...
{
/* HINT: If bShowBoat is true, display the actual contents
of array element, a 'V' */
/* HINT: else, display a wave character to kee the Battle
Boat location secret, '~' */
if (bShowBoat == true)
{
std::cout << 'V' << "|";
}
...
{
/* HINT: Display the character actually present in the
array element */
...
// /\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\
/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\/\ //
if (isOnTarget(iRow, iColumn) == true)
{
this->showSunk();
...
{
this->displayGrid(false);
std::cout << "Location row: " << iRow << "; column: " << iColumn
<< " is close!" << std::endl;
}
else
{
...
/* ?? call member function to dislay the grid and show the
location of the battle boat ?? */
